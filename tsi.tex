...
x_{40} &= \chi_{I} = \arctan\left(\dfrac{\dot{\lambda}}{\dot{\delta}}\right)=\arctan\left(\dfrac{x_{45}}{x_{24}}\right)=\text{atan2}\left(x_{45},x_{24}\right)\\
\end{split}
\end{equation} 
...
With the required angles known, \Cref{eq:inVelToRotVel} can be written as an auxiliary function as well including the required transformation angles. These expressions are provided in \Cref{eq:rotTransAnglAux} with the extra auxiliary equations represented by \Cref{eq:rotExtraTransAnglAux}. Here $\dot{\tau}$ was found by noticing that $r\dot{\tau}=r\dot{\lambda}-\Omega_{M}r\cos \delta$, also see \Cref{fig:rotational_inertial_martian_velocity,fig:flight_path_and_azimuth_angle}.
...
x_{13} &= \chi_{R} = \arctan\left(\dfrac{\dot{\lambda}}{\dot{\delta}}\right)=\arctan\left(\dfrac{x_{46}}{x_{24}}\right)=\text{atan2}\left(x_{46},x_{24}\right)\\
\end{split}
\end{equation} 
...
&
x_{46} &= x'_{11} = \dot{\tau} = x_{45}-\Omega_{M}\cos x_{12}
\end{align}  
...
\begin{split}
x'_{10} &= 0 \\
x'_{11} &=  \dot{\tau} = x_{45}-\Omega_{M}\cos\left(x_{12}\right)\\
x'_{12} &= \dot{\delta} = \dfrac{x_{20}x_{6}-x_{3}x_{25}}{x_{8} \sqrt{1-\left(\dfrac{x_{3}}{x_{20}}\right)^{2}}}\\
x'_{13} &= \dfrac{x_{24}x'_{46}-x_{46}x'_{24}}{x_{46}^{2}x_{24}^{2}}\\
...
x'_{35} &=  \Omega_{M}\left(\cos x_{12}x_{25}-x_{18}\sin x_{12}\right) \\
x'_{36} &=  \dfrac{x'_{21}}{2x_{36}} \\
x'_{37} &= \dfrac{x_{36}x'_{25}-x_{25}x'_{36}}{x_{36}^{2}} \\
x'_{38} &= \dfrac{x'_{37}}{\sqrt{1-x_{37}^{2}}} \\ 
x'_{40} &=  \dfrac{x_{24}x'_{45}-x_{45}x'_{24}}{x_{45}^{2}x_{24}^{2}}\\
...
x'_{46} &= x''_{11} = x'_{45}+\Omega_{M}\sin x_{12}x_{24} \\
\end{split}
\end{align}
...
u_{10} &= 0 \\
u_{11} &= \dot{\tau}=\dfrac{x_{1}x_{5}-x_{2}x_{4}}{x_{19}}\\
\end{split}
&
\begin{split}
u_{12} &= \dot{\delta} = \dfrac{x_{20}x_{6}-x_{3}x_{25}}{x_{8} \sqrt{1-\left(\dfrac{x_{3}}{x_{20}}\right)^{2}}}\\
u_{13} &= \dfrac{x_{24}u_{46}-x_{46}u_{24}}{x_{46}^{2}x_{24}^{2}}\\
...
&
\begin{split}	
u_{40} &=  \dfrac{x_{24}u_{45}-x_{45}u_{24}}{x_{45}^{2}x_{24}^{2}}\\
...
u_{46} &= u_{45}+\Omega_{M}\sin x_{12}x_{24} \\
\end{split}
\end{align}
...
%\mathbb{T}_{\mathbf{IB}}=
%\begin{bmatrix}
%c\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right)\left(-s\delta c\chi c\gamma +c\delta s\gamma \right)-s\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) s\chi c\gamma  & c\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) s\delta s\chi -s\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) c\chi & c\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right)\left(-s\delta c\chi s\gamma -c\delta c\gamma \right)-s\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) s\chi s\gamma \\
%s\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right)\left(-s\delta c\chi c\gamma +c\delta s\gamma \right)+c\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) s\chi c\gamma  & s\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) s\delta s\chi +c\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) c\chi & s\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right)\left(-s\delta c\chi s\gamma -c\delta c\gamma \right)+c\left(\Omega_{M}t_{O}-\omega_{P}+\tau\right) s\chi s\gamma \\
...
3 & $ x_{12}, x_{25}, x_{31} $ & 11 &$ x_{16}, x_{32} $ & 17 & $ u_{25}, u_{30}, u_{34}, u_{36}, u_{40}, u_{46} $ & 25 & $ u_{16}, u_{27} $ \\ \hline
4 & $ x_{24}, x_{30}, x_{34}, x_{35}, x_{37}, x_{46} $ & 12 & $ x_{29} $ & 18 & $ u_{13}, u_{24}, u_{37}, u_{41} $ & & $  $ \\ \hline
5 & $ x_{13}, x_{28}, x_{33}, x_{38}, x_{40}, x_{41} $ & 13 & $ x_{27} $ & 19 & $ u_{38} $ & & $  $ \\ \hline
6 & $ x_{42} $ & 14 & $ w_{4,2} $ & 20 & $ u_{42} $ & & $  $ \\ \hline
...
$w_{13,1}=$ & $ x_{24}u_{46} $ & Multiplication \\ \hline
$w_{13,2}=$ & $ x_{46}u_{24} $ & Multiplication \\ \hline
$w_{13,3}=$ & $ x_{46}^{2} $ & Power \\ \hline
...
$w_{40,1}=$ & $ x_{24}u_{45} $ & Multiplication \\ \hline 
$w_{40,2}=$ & $ x_{45}u_{24} $ & Multiplication \\ \hline 
$w_{40,3}=$ & $ x_{45}^{2} $ & Power \\ \hline
...
$w_{46}=$ & $ w_{4,4}x_{24} $ & Multiplication \\ \hline 
% $w_{•,•}=$ & $  $ & \\ \hline 
\end{longtable}
...
&=\dfrac{1}{kx_{22}} \displaystyle\sum_{j=0}^{k-1}\left[2k-j\left(2+1\right)\right] \dfrac{U_{24}\left(k-j-1\right)}{k-j} W_{13,4}\left(j\right) \\
...
\begin{split}
U_{11} \left(k\right)&= X_{45} \left(k\right)-\Omega_{M}W_{4,7}\left(k\right) = \dfrac{U_{45} \left(k-1\right)}{k}-\Omega_{M}W_{4,7}\left(k\right) \\
...
\begin{split}
U_{35} \left(k\right)&= \Omega_{M}\left(W_{35,1}\left(k\right)-W_{35,2}\left(k\right)\right) \\ 
...
U_{46} \left(k\right)&= U_{45} \left(k\right)+\Omega_{M}W_{46}\left(k\right) \\
\end{split}
\end{align}
